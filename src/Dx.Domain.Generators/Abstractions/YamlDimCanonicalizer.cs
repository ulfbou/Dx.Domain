    ...
    public sealed class YamlDimCanonicalizer : IDimCanonicalizer
    {
            ...
            string filePath)
        {
            try
            {
                ...
                if (raw == null)
                    return Result<(string, DomainIntentModel)>.Failure(
                        DomainError.Create("DX-DIM-001", $"Empty or invalid YAML in {filePath}"));

                // 2. Validate Version (Phase 1 Gate)
                if (!raw.ModelVersion.StartsWith("1.", StringComparison.Ordinal))
                    return Result<(string, DomainIntentModel)>.Failure(
                        DomainError.Create("DX-DIM-002", $"Unsupported modelVersion '{raw.ModelVersion}' in {filePath}. Only 1.x is supported."));
                ...
                return Result<(string, DomainIntentModel)>.Success((canonicalJson, model));
            }
            catch (Exception ex)
            {
                // We trap parser errors and convert them to proper Domain Errors
                return Result<(string, DomainIntentModel)>.Failure(
                    DomainError.Create("DX-DIM-000", $"Canonicalization failed for {filePath}: {ex.Message}"));
            }
        }
