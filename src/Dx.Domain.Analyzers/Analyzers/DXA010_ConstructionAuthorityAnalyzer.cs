...
private static readonly LocalizableString MessageFormat =
    "Create domain instances via the Dx facade. Direct construction or public factory use is forbidden outside runtime substrate packages.";
...
public override void Initialize(AnalysisContext context)
{
    ...
    context.RegisterCompilationStartAction(startContext =>
    {
        ...
        {
            // 1. Runtime substrate boundary – must be first.
            if (IsRuntimeSubstrateInternal(operationContext.Compilation.Assembly, services))
                return;
...
private static bool IsRuntimeSubstrateInternal(IAssemblySymbol assembly, AnalyzerServices services)
{
    if (services.Scope.IsKernelInternal(assembly))
        return true;

    return assembly.Name.Equals("Dx.Domain.Primitives", StringComparison.Ordinal) ||
           assembly.Name.StartsWith("Dx.Domain.Primitives.", StringComparison.Ordinal) ||
           assembly.Name.Equals("Dx.Domain.Facts", StringComparison.Ordinal) ||
           assembly.Name.StartsWith("Dx.Domain.Facts.", StringComparison.Ordinal);
}
