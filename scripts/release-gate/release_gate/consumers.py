...
FIXTURES = {
    ...
    "combined_valid": "using System; using Dx.Domain.Annotations; using Dx.Domain.Kernel; using Dx.Domain.Primitives; using Dx.Domain.Facts;\n[Entity] sealed class ConsumerEntity { } var id = CorrelationId.New(); var result = Dx.Result.Success(id); var fact = Fact<CorrelationId>.Create(\"consumer.combined\", result.Value, default); Console.WriteLine(fact.GetPayload());\n",
...
def create_isolated_workspace(base_evidence_dir: Path, case: dict, candidate_dir: Path, candidate_manifest: dict) -> ConsumerWorkspace:
    ...
    output = "" if case["targetFramework"] == "netstandard2.0" else "<OutputType>Exe</OutputType>"
    project = f'<Project Sdk="Microsoft.NET.Sdk"><PropertyGroup><TargetFramework>{case["targetFramework"]}</TargetFramework>{output}<ImplicitUsings>enable</ImplicitUsings><Nullable>enable</Nullable><RestorePackagesWithLockFile>false</RestorePackagesWithLockFile><DirectoryBuildPropsPath>{html.escape(str(props.resolve()))}</DirectoryBuildPropsPath><ManagePackageVersionsCentrally>false</ManagePackageVersionsCentrally></PropertyGroup><ItemGroup>{references}</ItemGroup></Project>\n'
